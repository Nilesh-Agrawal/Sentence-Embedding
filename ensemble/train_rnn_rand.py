...
class Dataset(object):
    ...
    def next(self, batch_size=None):
        ...
        batch_x1_sif = (np.random.randint(1,high=40000,size=(batch_size,300)))
        
        batch_x2_sif = (np.random.randint(1,high=40000,size=(batch_x1.shape[0],300)))
    ...
    for step in range(1, training_steps + 1):
        ...
        if step % display_step == 0 or step == 1:
            ...
            print("Time: " + strftime("%Y-%m-%d %H:%M:%S", gmtime()) + ", Step " + str(step*batch_size) + ", Loss= " + \
                  ...
                  "{:.5f}".format(acc))
            if prev_loss > loss:
                save_path = saver.save(sess, "./tmp_rand_rnn/model",global_step=1000)
